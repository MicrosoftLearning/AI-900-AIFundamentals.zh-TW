...
1. 在 [ Azure Machine Learning 工作室 ](https://ml.azure.com?azure-portal=true) 中，選取畫面左上方的功能表圖示，以展開左窗格。 選取 [ ** 資料] ** 頁面（在 [資產 ** ] 底下 ** ）。 [資料] 頁面具有您計劃在 Azure ML 中使用的特定資料檔案或資料表。 您也可以從這個頁面建立資料集。

1. 在 [ ** 資料] 頁面上的 [資料 ** 資產 ** ] 索引標籤底下 ** ，選取 ** [+ 建立 ** ]。 然後使用下列設定來設定資料資產：
    - **資料類型**：
        - **名稱**：diabetes-data
        - **描述**：糖尿病資料
        - **資料集類型**：表格式
...
1. 在 [ Azure Machine Learning 工作室中，在左窗格中選取 ** [設計工具 ** ] 專案（在 [撰寫 ** ] 底下 ** ），然後選取 ** + ** 以建立新的管線。 ](https://ml.azure.com?azure-portal=true)
...
1. 以滑鼠右鍵按一下畫布上的糖尿病資料 ** 資料集（Ctrl+按一下 Mac）， ** 然後選取 [ ** 預覽資料 ** ]。
...
1. 尋找 [ ** 選取資料集 ** 中的資料行] 模組，並將其放在糖尿病資料 ** 資料集下方的 ** 畫布上。 然後，將 **diabetes-data** 資料集底部的輸出連線至 [選取資料集中的資料行]**** 模組頂端的輸入。

1. 按兩下 [選取資料集的資料行]**** 模組，以存取右側的設定窗格。 選取 [編輯資料行]****。 然後在 [ ** 選取資料行] ** 視窗中，選取 [依名稱 ** ] 和 ** [ ** 新增所有資料 ** 行]。 然後移除 ** PatientID ** ，然後按一下 [ ** 儲存 ** ]。

1. **尋找標準化資料 ** 模組，並將它放在 [選取資料集 ** 中的資料行] 模組下方 ** 的畫布上。 然後，將 [選取資料集中的資料行]**** 模組底部的輸出連線至 [正規化資料] **** 模組頂端的輸入，如下所示：
...
1. 將 [轉換方法]** 設定為 **MinMax**，並將 [檢查時常數資料行使用 0]** 設定為 **True**。 使用 [編輯資料行] 編輯要轉換的資料 ** 行 ** 。 選取 [具有規則的資料 ** 行 ** ]，然後複製下列清單，並貼到 [包含資料行名稱] 底下：  
...
1. 在 [ ** 基本] ** 頁面上，選取 ** [新建 ** ]，並將實驗的名稱設定為 ** mslearn-diabetes-training ** ，然後選取 [ ** 下一步 ** ]。
...
1. 請等候幾分鐘，讓流程執行完成。 您可以選取 ** [資產 ** ] 底下的 ** [作業] 來檢查作業 ** 的狀態。 從該處選取 mslearn-diabetes-training ** 實驗，然後 ** 選取 ** [糖尿病訓練 ** ] 工作。

## 檢視轉換的資料

當執行完成時，資料集現在已準備好進行模型定型。

1. 以滑鼠右鍵按一下 [Mac] 上的 ** [標準化資料 ** ] 模組，然後選取 [ ** 預覽資料 ** ]。 選取 [轉換的資料集]****。
...
1. 選取 [ ** 設定及提交 ** ]，然後使用名為 ** mslearn-diabetes-training ** 的現有實驗來執行管線。

1. 等待實驗執行完畢。 這可能需要 5 分鐘或更久的時間。

1. 選取 ** [資產 ** ] 底下的 ** [作業]，以檢查作業 ** 的狀態。 從該處選取 ** mslearn-diabetes-training ** 實驗，然後選取最新的 ** 糖尿病訓練 ** 工作。

1. 在新的索引標籤上，以滑鼠右鍵按一下 [Mac ** ] 畫布上的 [評分模型] ** 模組，選取 [預覽資料 ** ]，然後選取 ** ** [評分資料集 ** ] 以檢視結果。
...
1. 在新索引標籤上，以滑鼠右鍵按一下畫布上的 [評估模型] ** 模組， ** 選取 [預覽資料 ** ]，然後選取 ** ** [評估結果 ** ] 以檢視效能計量。 這些計量可協助資料科學家評估模型根據驗證資料進行預測的效能。
...
1. 針對從自訂資料集建立的模型，管線不會自動包含 **Web 服務輸入**元件。 從資產庫搜尋 **Web 服務輸入**元件，並放在管線頂端。 連線 的 ** 輸出已在畫布上的 [選取資料集 ** 中的資料行] 元件中的 [Web 服務輸入 ** ] 元件 ** 。
...
1. 連線 的 ** 輸出評分模型 ** 模組至 * 執行 Python 腳本 ** 的 ** Dataset1 * （最左邊）輸入，並將執行 Python 腳本 ** 模組的結果資料集 * （左）輸出 ** 連接到 * ** Web 服務輸出 ** 。
...
1. 返回 [作業] ** 索引 ** 標籤。從該處選取 ** mslearn-diabetes-inference ** 實驗，然後選取 [ ** 預測糖尿病 ** ] 工作。
