...
Azure Machine Learning 的某些功能處於預覽狀態，而且必須在工作區中明確啟用。
...
自動化機器學習可讓您嘗試多個演算法和參數來定型多個模型，並找出最適合您資料的演算法和參數。 在此練習中，您將使用歷史自行車出租詳細資料的資料集來定型模型，以根據季節性和天氣特徵預測應該在特定日期預期的自行車租用數目。
...
1. 在[[Azure Machine Learning 工作室](https://ml.azure.com?azure-portal=true)] 中，檢視 [**自動 ML**] 頁面 ([**撰寫**) ]。

1. 使用下列設定建立新的自動化 ML 作業，並視需要 **使用 Next** 來逐步執行使用者介面：
   ...
   **工作類型 & 資料**：
    ...
    **工作設定**：
    ...
    - **限制**： *展開本節*
        ...
        - **並行試用版上限**：3
        - **最大節點**：3
        - **計量分數閾值**：0.85 (*，讓模型達到標準化根平均平方誤差計量分數 0.085 或更少時，作業就會結束。*) 
        - **逾時**：15
        - **反復專案逾時**：5
        - **啟用早期終止**： *已選取*
    - **驗證和測試**：
...
1. 選取 [計量] 索引標籤，然後選取**殘差**與 **predicted_true** 圖表 (如果尚未選取)。 
    ...
    檢閱顯示模型效能的圖表。 **殘差**圖表會顯示殘*差* (預測值與實際值之間的差異，) 為長條圖。 **predicted_true**圖表會比較預測值與 true 值。 

## 部署和測試模型

1. 在自動化機器學習作業所定型最佳模型的 **[模型** ] 索引標籤上，選取 [ **部署** 並使用 **Web 服務** ] 選項來部署具有下列設定的模型：
    - **名稱**：predict-rentals
    - **描述**：預測自行車租借
    - **計算類型**：Azure 容器執行個體
    - **啟用驗證**： *已選取*
