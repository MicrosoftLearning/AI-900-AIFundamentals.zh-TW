...
1. 請等候幾分鐘，讓流程執行完成。 您可選取 [資產]**** 下的 [作業]**** 來檢查作業狀態。 從該處選取 ** [自動價格訓練 ** ] 作業。 您可以從這裡查看作業完成的時間。 作業完成後，資料集現在已準備好進行模型定型。
...
1. 在 [建立推斷管線] **** 下拉式清單，選取 [即時推斷管線]****。 幾秒鐘後，就會開啟名為 **Auto Price Training-real time inference** 的新版本管線。
