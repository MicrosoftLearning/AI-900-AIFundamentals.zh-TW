...
1. 在 Azure Machine Learning 工作室中，您應該會看到新建立的工作區。 如果情況並非如此，請在左側功能表中選取您的 Azure 目錄。 然後，從新的左側功能表中，選取 [ ** 工作區 ** ]，其中會列出與您目錄相關聯的所有工作區，然後選取您為此練習建立的工作區。
...
1. 在 [ Azure Machine Learning 工作室 ](https://ml.azure.com?azure-portal=true) 中，選取 ** &#8801; ** 圖示（左上方看起來像三行堆疊的功能表圖示），以檢視介面中的各種頁面（您可能需要最大化螢幕大小）。 您可以使用左側窗格中的頁面來管理工作區中的資源。 選取 [計算]**** 頁面 (在 [管理]**** 下)。
...
1. 在 [ Azure Machine Learning 工作室 ](https://ml.azure.com?azure-portal=true) 中，選取畫面左上方的功能表圖示，以展開左窗格。 檢視 [設計工具] 頁面 （在 ** [撰寫 ** ] 下 ** ），然後選取 ** + ** 以建立新的管線。 **

1. 將草稿名稱 （* * Pipeline-Created-on-date * ***） 變更為 ** 自動價格訓練 ** 。
...
1. 選取 [ ** 元件 ** ]。 搜尋並將 [汽車價格資料 (原始)]**** 資料集放在畫布上。

1. 以滑鼠右鍵按一下 （Ctrl+按一下 Mac） ** 畫布上的汽車價格資料 （Raw） ** 資料集，然後選取 [ ** 預覽資料 ** ]。
...
1. 關閉 [ ** DataOutput ** ] 視窗，讓您可以在畫布上看到資料集，如下所示：
...
1. 按兩下 [ ** 清除遺漏的資料] ** 模組，然後在右側的窗格中，選取 [ ** 編輯] 資料行 ** 。 然後在 [要清除的資料行]**** 視窗中，選取 [套用規則]****，在 [包括]**** 清單中選取 [資料行名稱]****，在資料行名稱的方塊中輸入 **bore**、**stroke** 和 **horsepower**，如下所示：
...
1. 按兩下 [正規化資料]**** 模組以檢視其參數窗格。 您會需要指定轉換方法，以及欲轉換的資料行。 將轉換方法設定為 **MinMax**。 選取 [編輯資料行]**** 以包含下列**資料行名稱**，藉此來套用規則：
    ...
    >**提示** 如果您比較 **stroke**、**peak-rpm** 及 **city-mpg** 資料行中的值，則會發現這些值全都以不同的刻度測量，且較大的 **peak-rpm** 值可能扭曲定型演算法，還比值較小的資料行 (例如 **stroke**) 更依賴此資料行。 一般而言，資料科學家會透過「正規化」** 數值資料行，以緩和這種可能的偏差，使其位於近似的量表。
...
1. 在 [ ** 基本] ** 頁面上，選取 ** [新建 ** ]，並將實驗的名稱設定為 ** mslearn-auto-training ** ，然後選取 [ ** 下一步 ** ]。
...
1. 請等候幾分鐘，讓流程執行完成。 您可以選取 ** [資產 ** ] 底下的 ** [作業] 來檢查作業 ** 的狀態。 從該處選取 ** [自動價格訓練 ** ] 作業。 您可以從這裡查看作業完成的時間。 作業完成後，資料集現在已準備好進行模型定型。

1. 移至左側功能表。 在 [撰寫] 底下 ** ** ，選取 [ ** 設計工具 ** ]。 然後從 [管線] 清單中選取您的 * 自動價格定 * 型管線 ** 。 **

## 建立定型管線

使用資料轉換準備資料之後，即可使用此資料來定型機器學習模型。 請完成下列步驟以延伸 **Auto Price Training** 管線。

1. 請確定左側功能表已 ** 選取 [設計工具 ** ]，且您已返回 ** [自動價格訓練 ** ] 管線。

1. 在左側 [資產庫]**** 窗格中，搜尋**分割資料**模組，並放在畫布上的 [正規化資料]**** 模組下方。 然後，將 [正規化資料]**** 模組的 [已轉換的資料集]** 輸出 (左側) 連線至 [分割資料]**** 模組的輸入。
...
1. 在 [資產庫]**** 中，搜尋**線性迴歸**模組，並放在畫布上的 [分割資料]**** 模組左側和 [定型模型]**** 模組上方。 然後將其輸出連線至 **Train Model** (定型模型) 模組的**未定型的模型** (左側) 輸入。
...
1. 選取 [ ** 設定及提交 ** ]，然後使用名為 ** mslearn-auto-training ** 的現有實驗來執行管線。

1. 實驗執行需要 5 分鐘以上才能完成。 返回 [ ** 作業] ** 頁面，然後選取最新的 ** 自動價格訓練 ** 作業執行。

1. 實驗執行完成時，以滑鼠右鍵按一下 ** 評分模型 ** 模組，然後選取 [預覽資料 ** ]，然後 ** 選取 ** [評分資料集 ** ] 以檢視結果。

1. 向右捲動，留意 **price** 資料行 (包含標籤的已知 true 值) 旁有一個名為 **Scored Labels** 的新資料行，其中包含預測的標籤值。

1. **關閉 [scored_dataset] 索引卷 ** 標。

模型正在預測 [price] (價格)**** 標籤的值，但預測的可靠程度為何？ 您必須評估模型才能評估可靠性。

## 評估模型

評估回歸模型的其中一種方法是比較預測標籤與定型期間所保留之驗證資料集中的實際標籤。 另一種方式是比較多個模型的效能。
...
1. 實驗執行需要幾分鐘的時間才能完成。 返回 [ ** 作業] ** 頁面，然後選取最新的 ** 自動價格訓練 ** 作業執行。

1. 當實驗執行完成時，請選取 [作業詳細資料]****，這會開啟另一個索引標籤。尋找**評估模型**模組並按一下滑鼠右鍵。 選取 [預覽資料]****，然後選取 [評估結果]****。

    ![評估模型位置的螢幕截圖。](media/create-regression-model/evaluate-model-help-1.png)
...
1. 在 [ ** 建立推斷管線 ** ] 下拉式清單中，選取 ** [即時推斷管線 ** ]。 幾秒鐘後，就會開啟名為 **Auto Price Training-real time inference** 的新版本管線。

1. 將新管線重新命名為 **Predict Auto Price**，然後檢查新的管線。 其中包含要提交之新資料的 Web 服務輸入，以及要傳回結果的 Web 服務輸出。 某些轉換和定型步驟是此管線的一部分。 定型的模型將用於評分新的資料。

    在下一個步驟中，您將對推斷管線進行下列變更：

    ![螢幕擷取畫面顯示已指定變更的推斷管線。](media/create-regression-model/inference-changes.png)

   當您在後續步驟中修改管線時，請使用映像進行參考。

1. 推斷管線假設新資料會符合原始定型資料的結構描述，所以包含來自定型管線的 **Automobile price data (Raw)** (汽車價格原始資料) 資料集。 不過，此輸入資料包括模型預測的 [價格]**** 標籤，而難以直接納入尚未預測價格的新車資料。 刪除此模組，並將它取代為 [資料輸入和輸出 ** ] 區段中的 ** [ ** 手動 ** 輸入資料] 模組。
1. 編輯 [ ** 手動 ** 輸入資料] 模組並輸入下列 CSV 資料，其中包含三輛車沒有標籤的功能值（複製並貼上整塊文字）：
...
1. 連線新的 **將 [手動 ** 資料] 模組輸入至與 Web 服務輸入 ** 相同的 ** [選取資料集中的資料行] 模組的 ** 資料集 ** ** 輸入 ** 。
...
1. 推斷管線包含的 [評估模型]**** 模組對於新資料預測並無幫助，因此請刪除此模組。 將輸出修改成僅包含預測：
    ...
    ```
>**注意 ** ：複製和貼上可能會將空格導入不應有的 Python 腳本中。 請仔細檢查匯入或解除 * 或 * * 傳回 * 之前 * * 沒有空格。 請確定scored_results和scored_results.rename（） * 之前 * * * 有一個索引標籤縮排。

1. 連線 的 ** 輸出將模型模組 ** 評分至 ** 執行 Python 腳本 ** 的 ** Dataset1 ** （最左邊）輸入。

1. 連線 **執行 Python 腳本 ** 模組的結果 ** 資料集 ** （左）輸出至 ** Web 服務輸出 ** 模組。

1. 確認您的管線看起來類似下列影像：

    ![汽車推斷管線的螢幕擷取畫面。](media/create-regression-model/inference-pipeline-lab.png)

1. 在計算叢集上，將管線提交為名為 **mslearn-auto-inference** 的新實驗。 該實驗可能需要一段時間執行。

1. 返回 [ ** 作業 ** ] 頁面，然後選取最新的 ** 自動價格訓練 ** 作業執行（與 mslearn-auto-inference 實驗相關的 * 作業 * ）。

1. 當管線完成時，以滑鼠右鍵按一下 [ ** 執行 Python 腳本] ** 模組。 選取 [預覽資料]****，然後選取 [結果資料集]****，以查看輸入資料中三部汽車的預測價格。

1. **關閉 [Result_Dataset] 索引卷 ** 標。
...
1. **predict-auto-price** 端點開啟時，選取 [測試]**** 索引標籤，稍後將用於測試模型與新資料。 刪除 [輸入資料] 底下的 ** 目前資料，以測試端點 ** 。 複製下列資料並貼至資料區段：  
...
>**注意 ** 刪除計算可確保您的訂用帳戶不會針對計算資源收費。 不過，只要您的訂用帳戶中有 Azure Machine Learning 工作區，您就必須為了資料儲存空間支付少量的費用。 如果您已完成探索 Azure Machine Learning，則可刪除 Azure Machine Learning 工作區及其相關的資源。 但是，如果您打算完成本系列中的任何其他實驗室，您將需要重新建立 Azure Machine Learning 工作區。
>
> 若要刪除您的工作區：
>
