...
1. 在 Azure Machine Learning 工作室中，您應該會看到新建立的工作區。 如果情況並非如此，請在左側功能表中選取您的 Azure 目錄。 然後，從新的左側功能表中選取 [工作區]****，其中會列出與您的訂用帳戶相關聯的所有工作區。
...
1. 在 Azure Machine Learning 工作室中，選取左上方的三條線，以檢視介面中各個頁面 (您可能需要將畫面放到最大)。 您可以使用左側窗格中的頁面來管理工作區中的資源。 選取 [計算]**** 頁面 (在 [管理]**** 下)。
...
1. 選取元件 搜尋並將 [汽車價格資料 (原始)]**** 資料集放在畫布上。
...
1. 請關閉 [diabetes-data 結果視覺效果]**** 視窗，以便在畫布上查看資料集，如下所示：
...
1. 請等候幾分鐘，讓流程執行完成。 您可以選取 ** [資產 ** ] 底下的 ** [作業] 來檢查作業 ** 的狀態。 從該處選取 ** [自動價格訓練 ** ] 作業。 您可以從這裡查看作業完成的時間。 當執行完成時，資料集現在已準備好進行模型定型。
...
1. 找出畫布上方的功能表，然後按一下 [建立推斷管線]****。 您可能需要將畫面展開為全螢幕，並按一下畫面右上角的三個點圖示 [...] ****，以在功能表中找到 [建立推斷管線]****。  

    ![螢幕擷取畫面顯示 [建立推斷管線] 的位置。](media/create-regression-model/create-inference-pipeline.png)

1. 在 [Create inference pipeline] (建立推斷管線)**** 下拉式清單中，按一下 [Real-time inference pipeline] (即時推斷管線)****。 幾秒鐘後，就會開啟名為 **Auto Price Training-real time inference** 的新版本管線。
...
>**注意** 停止您的計算可確保您的訂用帳戶不需支付計算資源的費用。 不過，只要您的訂用帳戶中有 Azure Machine Learning 工作區，您就必須為了資料儲存空間支付少量的費用。 如果您已完成探索 Azure Machine Learning，則可刪除 Azure Machine Learning 工作區及其相關的資源。 但是，如果您打算完成本系列中的任何其他實驗室，您將需要重新建立 Azure Machine Learning 工作區。
>
> 若要刪除您的工作區：
>
