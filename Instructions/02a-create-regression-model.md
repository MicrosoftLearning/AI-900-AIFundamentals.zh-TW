...
1. 請等候幾分鐘，讓流程執行完成。 您可以選取 **[資產**] 底下的 [**作業**] 來檢查作業的狀態。 從該處選取 **[自動價格訓練** ] 作業。
...
1. 當實驗執行完成時，以滑鼠右鍵按一下 **評分模型** 模組，然後選取 **[預覽資料** ]，然後選取 **[評分資料集** ] 以檢視結果。

1. 向右捲動，留意 **price** 資料行 (包含標籤的已知 true 值) 旁有一個名為 **Scored Labels** 的新資料行，其中包含預測的標籤值。

1. 關閉 **[scored_dataset] 索引** 標籤。
...
1. 實驗執行完成時，以滑鼠右鍵按一下 **[評估模型]** 模組，然後選取 **[預覽資料** ]，然後選取 [ **評估結果**]。
