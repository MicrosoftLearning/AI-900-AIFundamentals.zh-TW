...
## 在 Designer 中建立管線，並新增資料集

Azure Machine Learning 包含可用於迴歸模型的範例資料集。

1. 在[Azure Machine Learning 工作室](https://ml.azure.com?azure-portal=true)中，選取畫面左上方的功能表圖示，展開左窗格。 檢視 [**撰寫**) ] 底下的 **[Designer**] 頁面 (，然後選取 **+** 以建立新的管線。
...
1. 以滑鼠右鍵按一下 (Ctrl+按一下 Mac，) 畫布上的 [原始) 資料集] (**[汽車價格資料 ** ]，然後選取 [ **預覽資料**]。
...
1. 按兩下 **[清除遺漏的資料** ] 模組，然後在右側的窗格中，選取 [ **編輯資料行**]。 然後在 [要清除的資料行] 視窗中，選取 [套用規則]，在 [包括] 清單中選取 [資料行名稱]，在資料行名稱的方塊中輸入 **bore**、**stroke** 和 **horsepower**，如下所示：
...
1. 在 [ **基本]** 頁面上，選取 [ **新建** ]，並將實驗名稱設定為 **mslearn-auto-training** ，然後選取 [ **下一步** ]。
...
1. 請等候幾分鐘，讓流程執行完成。 您可以選取 [**資產**] 底下的 [**作業**]，以檢查作業的狀態。 從該處選取 **[自動價格訓練** ] 作業。 您可以從這裡查看作業何時完成。 作業完成後，資料集現在已準備好進行模型定型。

1. 移至左側功能表。 在 **[撰寫]** 底下，選取 **[Designer**]。 然後從管線清單中選取您的*自動價格訓練***管線**。

## 建立定型管線

使用資料轉換準備資料之後，即可使用此資料來定型機器學習模型。 請完成下列步驟以延伸 **Auto Price Training** 管線。

1. 請確定左側功能表**已選取Designer**，且您已返回 **[自動價格訓練**] 管線。
...
1. 選取 [ **設定 & 提交**]，然後使用名為 **mslearn-auto-training**的現有實驗來執行管線。

1. 實驗執行需要 5 分鐘以上才能完成。 返回 [ **作業]** 頁面，然後選取最新的 **自動價格訓練** 作業執行。

1. 實驗執行完成時，以滑鼠右鍵按一下 **評分模型** 模組，然後選取 **[預覽資料** ]，然後選取 [ **評分資料集** ] 以檢視結果。
...
評估回歸模型的其中一種方式是比較預測標籤與定型期間所保留之驗證資料集中的實際標籤。 另一種方式是比較多個模型的效能。
...
1. 推斷管線假設新資料會符合原始定型資料的結構描述，所以包含來自定型管線的 **Automobile price data (Raw)** (汽車價格原始資料) 資料集。 不過，此輸入資料包括模型預測的 [價格] 標籤，而難以直接納入尚未預測價格的新車資料。 刪除此模組，並將它取代為 [**資料輸入和輸出**] 區段中的 **[手動輸入資料**] 模組。
1. 編輯 **[手動輸入資料** ] 模組並輸入下列 CSV 資料，其中包含三個汽車的功能值， (複製並貼上整個文字區塊) ：
...
1. 推斷管線包含的 [評估模型] 模組對於新資料預測並無幫助，因此請刪除此模組。 將輸出修改成僅包含預測：
    ...
    ```
>**注意**：複製和貼上可能會將空格導入不應存在 Python 腳本中。 在 *匯* 入或 *def* 或 *return*之前，請仔細檢查是否有空格。 在 *scored_results* 和 *scored_results.rename () *之前，請確定有一個索引標籤縮排。

1. 將 **[評分模型**] 模組的輸出連線至 [**執行 Python 腳本**] (最左邊) 輸入的**Dataset1**。

1. 將執行**Python 腳本**模組 (左側) 輸出**的結果資料集**連接到**Web 服務輸出**模組。
...
1. 返回 [**作業**] 頁面，然後選取與*mslearn-auto-inference 實驗*) 相關的最新**自動價格定**型作業執行 (。

1. 當管線完成時，以滑鼠右鍵按一下 **[執行 Python 腳本]** 模組。 選取 [預覽資料]，然後選取 [結果資料集]，以查看輸入資料中三部汽車的預測價格。

1. 關閉 **[Result_Dataset] 索引** 標籤。
...
1. 在 [ **預測自動價格** 作業執行] 頁面中，從頂端功能表列選取 [ **部署** ]。
    ...
    ![螢幕擷取畫面顯示「預測自動價格」推斷管道的部署按鈕。](media/create-regression-model/deploy-screenshot.png)
...
1. **predict-auto-price** 端點開啟時，選取 [測試] 索引標籤，稍後將用於測試模型與新資料。 刪除 [ **輸入資料] 底下的目前資料以測試端點**。 複製下列資料並貼至資料區段：  
