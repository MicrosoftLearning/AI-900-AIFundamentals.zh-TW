...
*Azure AI 視覺 * 服務提供實用的預先建置模型來使用影像，但您通常需要為電腦視覺定型自己的模型。 例如，假設野生動物保護組織想要使用運動敏感相機來追蹤動物的目擊。 然後，攝像機拍攝的圖像可以用來驗證特定區域的特定物種的存在，並協助保護瀕危物種。 為了達成此目的，組織將受益于 * 經過訓練以識別所擷取相片中不同動物物種的影像分類 * 模型。

在 Azure 中 *** ，您可以使用 自訂視覺 *** 服務，根據現有的影像來定型影像分類模型。 建立影像分類解決方案有兩個要素。 首先，您必須訓練模型，以使用現有影像來辨識不同的類別。 然後，模型完成定型時，您必須將其發佈為服務以供應用程式使用。

為了測試自訂視覺服務的功能，我們會使用在 Cloud Shell 中執行的簡單命令列應用程式。 相同的原則和功能適用于真實世界的解決方案，例如網站或行動應用程式。
...
>**注意** 並非所有區域都有提供這些資源。 不論您建立自訂視覺還是 Azure AI 服務資源，只有在特定 [ 區域中 ](https://azure.microsoft.com/global-infrastructure/services/?products=cognitive-services) 建立的資源可用來存取自訂視覺服務。 為了簡單起見，以下設定指示會預先為您選取某個區域。
...
1. 從 [https://aka.ms/animal-images](https://aka.ms/animal-images) 下載並擷取正在訓練的影像。 這些影像是在壓縮的資料夾中提供，當擷取時會包含稱為 ** 大象 ** 、 ** 長頸鹿 ** 和 ** 獅子 ** 的子資料夾。
...
1. 按一下 [ ** 新增影像 ** ]，然後選取您先前擷取的 ** 大象 ** 資料夾中的所有檔案。 然後上傳影像檔案，並指定標記 * 大象 * ，如下所示：

    ![影像上傳介面的螢幕擷取畫面。](media/create-image-classification-system/upload-elephants.png)

1. 使用 [ ** 新增影像 ** ][+]] 按鈕，以標記長頸鹿上傳長頸 * 鹿 ** 資料夾中的影像 ** ，並使用標記 * * 獅子 * 在獅子 ** 資料夾中上傳影像 ** 。

1. 探索您在自訂視覺專案中上傳的影像 - 每個類別應該有 17 個影像，如下所示：

    ![自訂視覺入口網站中已標記訓練影像的螢幕擷取畫面。](media/create-image-classification-system/animal-training-images.png)

1. 在自訂視覺專案中，於影像上方按一下 [定型]**** 以使用已加上標籤的影像來定型分類模型。 選取 [ ** 快速訓練] ** 選項，然後等候定型反復專案完成。

    > **提示 ** ：訓練可能需要幾分鐘的時間。 在等待時，請查看 [ 雪豹自拍和 AI 如何協助拯救物種免于滅絕 ](https://news.microsoft.com/transform/snow-leopard-selfies-ai-save-species/) ，其中描述了一個使用電腦視覺來追蹤野生瀕危動物的真實專案。
...
1. 檢視模型傳回的預測 - 長頸鹿 * 的 * 機率分數應該是最高的，如下所示：
...
1. 按一下 [&#128504; 發佈]****，使用下列設定發佈已定型的模型：
    - **模型名稱 ** ：動物
    - **預測資源 ** ： * 您先前 * 建立的 Azure AI 服務或自訂視覺預測資源。
...
現在您可以使用範例用戶端應用程式，根據它們所包含的動物來分類影像。
