...
*電腦視覺 * 服務會使用預先定型的機器學習模型來分析影像，並擷取其相關資訊。
...
您可以藉由建立 ** 電腦視覺資源或 ** Azure AI 服務資源，來使用電腦視覺 ** 服務 ** 。
...
1. 不要太擔心程式碼，重要的是它需要端點 URL 和 Azure AI 服務資源的其中一個金鑰。 從 Azure 入口網站資源的 [金鑰和端點]**** 頁面複製上述資訊，並貼到程式碼編輯器中，分別取代 **YOUR_KEY** 和 **YOUR_ENDPOINT** 預留位置值。
