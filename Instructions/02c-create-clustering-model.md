...
1. 在 [Azure Machine Learning 工作室](https://ml.azure.com?azure-portal=true) 中，選取 **&#8801;** 圖示（左上方看起來像三行的功能表圖示），以檢視介面中的各種頁面（您可能需要最大化螢幕大小）。 您可以使用左側窗格中的頁面來管理工作區中的資源。 選取 [計算]**** 頁面 (在 [管理]**** 下)。
...
1. 在 [**數據] 頁面上的 [數據**資產 **] 索引標籤底下**，選取 **[+ 建立**]。 然後使用下列設定來設定資料資產：
    - **資料類型**：
        - **名稱**：penguin-data
        - **描述**：Penguin 資料
        - **資料集類型**：表格式
...
1. 在 [ **運行時間設定** ] 頁面上，出現錯誤，因為您沒有執行管線的默認計算。 在 [ **選取計算類型** ] 下拉式清單中選取 *[計算叢集* ]，然後在 [ **選取 Azure ML 計算叢集] 下拉式清單中選取您最近建立的計算叢集** 。
